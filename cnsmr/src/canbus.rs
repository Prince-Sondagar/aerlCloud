...
pub async fn from_frame(
    ...
    if let Some(device) = KNOWN_DEVICES.iter().find(|d| (d.matcher)(frame.id)) {
        ...
        for msg in dbc.messages() {
            ...
            } else if msg.message_id().0 == frame.id {
                for sig in msg.signals() {
                    let signal = parse_signal(frame, sig).unwrap();
...
// TODO generalise below and add SRX support.
fn parse_signal(frame: &Frame, sig: &Signal) -> Result<f64> {
    // Load bits into reader
    let mut bits = LittleEndianReader::new(&frame.data);

    // Discard bits before start
    if *sig.start_bit() != 0u64 {
        bits.read_bits(*sig.start_bit() as u32);
    }

    // Get the bits we're interested in.
    let signal = bits.read_bits(*sig.signal_size() as u32).unwrap();

    let factored_signal = signal as f64 * sig.factor();
    let offset_signal = factored_signal + sig.offset();
