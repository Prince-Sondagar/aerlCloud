...
lazy_static::lazy_static! {
    static ref SRX: Device = Device {
        ...
        matcher: |id| {
            let identifier = Identifier::from_raw(id);
            let device = identifier.device;
            let message = identifier.message;

            (device >= 10 && device <= 51) && message <= 37
        },
    };
    static ref EG: Device = Device {
        ...
        matcher: |id| {
            ...
            let device = identifier.device;
            let message = identifier.message;
            (device >= 52 && device <= 55) &&  message <= 37
        },
    };

    static ref PYLONTECH: Device = Device {
        dbc: can_dbc::DBC::from_slice(include_str!("../dbc/pylontech_lv.dbc").as_bytes()).unwrap(),
        matcher: |id| {
            matches!(id, 773 | 862 | 860 | 854 | 853 | 849 | 857)
        },
    };

    pub static ref DEVICES: Vec<Device> = vec![
        PYLONTECH.to_owned(),   // Hacky fix: SRX matcher matches pylontech
                                //packets, so we check pylontech first by
                                // putting it at the start of the vector.
        SRX.to_owned(),
        EG.to_owned(),
