...
const EarthAnimation = (props: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    function drawPoint(
      ...
      context.beginPath();
      context.fillStyle = theme?.colors.primary ?? "grey";
    ...
    if (canvasRef.current) {
      ...
      if (context) {
        ...
        context.fillStyle = theme?.colors.textLight ?? "rgb(75, 75, 75)";
        context?.fill();
        context.lineWidth = 0.5;
        context.strokeStyle = theme?.colors.border ?? "rgb(25, 25, 25)";
        context?.stroke();
  ...
  return (
    <div className="max-w-screen max-h-screen overflow-hidden">
      <div className="w-screen	h-screen absolute z-40">{props.children}</div>
      <div className="sm:col-span-12 xs:col-span-0">
        <canvas
