...
serve(async (req: any) => {
  ...
  try {
    ...
    if (!orgInviteUser?.length) {
      return genResponse(
        false,
        "You are not logged in or you have already accepted invitation.",
      );
    }
    ...
    if (removeUserFromOrgInvite?.length > 0) {
      ...
      return genResponse(false, "Joining Organization Failed");
    }
...
// Check whether a user is in an org and has permissions.
