...
pub async fn task() -> Result<()> {
    ...
    tokio::spawn(async move {
        let ingest: Ingest<Payload> = Ingest::new(format!("{}/stream/canbus/push", &ARGS.api_url));
        ...
        loop {
            ...
            // any pending items
            if !q.is_empty() {
                let payload = Payload {frames: q.clone()};
                ingest.push(payload).await;
                q.clear();
            } else {
                log::warn!("[canbus] No CAN bus frames to send.");
            }
    ...
    loop {
        log::debug!("[canbus] Cleaning up CAN Bus handlers");
        // remove threads that have ended
        can_threads.retain(|_, t| !t.is_finished());

        // get interfaces
        let interfaces = interfaces();
        log::info!("[canbus] Found {} CAN Bus interfaces", interfaces.len());
        ...
        for interface in interfaces {
            ...
            if !can_threads.contains_key(&interface) {
                log::debug!("[canbus] Opening interface: {interface}");

                if let Err(e) = configure(&interface) {
                    log::error!("[canbus] Failed to configure interface {interface} with error {e}");
                    continue;
                }
                ...
                let can = match CanSocket::open(&interface) {
                    Ok(socket) => socket,
                    Err(e) => {
                        log::error!("[canbus] Could not open socket: {e}");
                        continue;
                    }
                };
                ...
                let handle = tokio::spawn(async move {
                    loop {
                        ...
                        let frame = match can.read_frame() {
                            Ok(frame) => frame,
                            Err(e) => {
                                log::error!("[canbus] Failed to read frame: {}", e);
                                continue;
                            }
                        };

                        log::debug!("Got frame with id: {:#x}", frame.id_word());
