...
pub async fn heartbeat(
    ...
    // Optionally include the fw_version if the device reports it
    let mut update_payload_map = serde_json::Map::new();
    update_payload_map.insert("last_seen".to_string(), json!(time_now_rfc3339()));
    if let Some(fw_version) = &payload.fw_version {
        update_payload_map.insert("fw_version".to_string(), json!(fw_version));
    }
    let update_payload = Value::Object(update_payload_map);
